...
users.get<{}, UserObject[]>(
  "/@all",
  async (req, res) => getUsers().then((users) => res.json(users))
  // .catch(() => res.status(500).send())
);

users.delete<{}, { message: string }>(
  "/@all",
  async (req, res) => deleteAllUsers().then(() => res.json({ message: "success" }))
  // .catch(() => res.status(500).send())
);

users.post<{}, { user: UserObject } | { message: string }, UserSignupInput>(
  "/@signup",
  async (req, res) =>
...
users.post<{}, { user: UserObject; token: string } | { message: string }, UserLoginInput>(
  "/@login",
  async (req, res) =>
